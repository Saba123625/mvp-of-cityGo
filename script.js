...
function addParkingMarker(spot) {
  ...
  marker.on("click", () => {
    if (!userLatLng) {
      alert("ჯერ ვერ ვპოულობ შენს მდებარეობას");
      return;
    }

    if (window.activeRoute) {
      map.removeControl(window.activeRoute);
    }

    window.activeRoute = L.Routing.control({
      waypoints: [userLatLng, L.latLng(spot.lat, spot.lng)],
...
fetch('data/parking.json')
  ...
  .then(data => {
    data.forEach((spot, index) => {
      const marker = addParkingMarker(spot);  
